...
let state = {
  ...
  diaryDraftVideo: [], // Draft array of video objects {src}
  ...
  appTitle: '?åÎûò??, // Custom app header title
...
const DEFAULT_CATEGORIES = {
  health: { label: 'Í±¥Í∞ï', color: '#10b981', class: 'cat-health-style' },
  family: { label: 'Í∞Ä??, color: '#f43f5e', class: 'cat-family-style' },
  school: { label: '?ôÍµê', color: '#0ea5e9', class: 'cat-school-style' },
  dev: { label: '?êÍ∏∞Í∞úÎ∞ú', color: '#a855f7', class: 'cat-dev-style' },
  exercise: { label: '?¥Îèô', color: '#f59e0b', class: 'cat-exercise-style' },
  other: { label: 'Í∏∞Ì?', color: '#6b7280', class: 'cat-other-style' }
};

// Recommended Emoji presets for each Tab Icon
const RECOMMENDED_EMOJIS = {
  search: ['?îç', '?îé', '??, '?îÆ', '??', '?îé', '?ëÅÔ∏?, '?ïµÔ∏?, '?¶Å', '?î•'],
  calendar: ['?ìÖ', '?ìÜ', '?óìÔ∏?, '?åô', '‚≠?, '??, '??, '?ÄÔ∏?, '??', '?éà'],
  todos: ['?éØ', '??, '?ìã', '?ìå', '?îî', '??', '?èÜ', '?íØ', '‚≠?, '?í´'],
  records: ['?ìù', '?çÔ∏è', '?ìî', '?ìì', '?í≠', '?èÔ∏è', '?é®', '?íå', '?ì∏', '?ß∏'],
  routines: ['?îÑ', '?æÔ∏è', '?ªÔ∏è', '?îÅ', '?í™', '?éØ', '?ôÔ∏è', '??, '?î•', '?ìÜ'],
  analytics: ['?ìä', '?ìà', '?í°', '??', '?î•', '?ìâ', '?ß¨', '?íé', '?ëë', '?éØ'],
  settings: ['?ôÔ∏è', '?îß', '?õ†Ô∏?, '?ß©', '?é®', '?îë', '?îí', '?îã', '?åê', '?õ∏']
};
...
function getCategory(categoryId) {
  ...
  const label = defaultLabels[categoryId] ? `${defaultLabels[categoryId]}(??†ú??` : '?ÜÏùå';
...
function init() {
  ...
  if (localStorage.getItem('neon_planner_gdrive_connected') === 'true') {
    ...
    if (savedToken) {
      ...
      if (badge) {
        badge.textContent = '?∞Í≤∞ ?ÑÎ£å (?êÎèô ?ôÍ∏∞??';
        badge.style.background = 'rgba(16, 185, 129, 0.15)';
        badge.style.color = '#10b981';
        badge.style.borderColor = '#10b981';
      }
      const info = document.getElementById('gdrive-user-info');
      if (info) info.textContent = 'Íµ¨Í? ?úÎùº?¥Î∏å ?§ÏãúÍ∞??ôÍ∏∞???ÅÌÉú';
    }
...
function loadFromLocalStorage() {
  ...
  if (savedAppTitle) {
    if (savedAppTitle === 'NEON PLANNER') {
      state.appTitle = '?åÎûò??;
      localStorage.setItem('neon_planner_app_title', '?åÎûò??);
    } else {
...
function triggerGDriveAutoSync() {
  ...
  gdriveSyncTimeout = setTimeout(async () => {
    ...
    if (statusBadge) {
      statusBadge.textContent = '?îÑ ?ôÍ∏∞??Ï§?..';
    ...
    try {
      ...
      if (searchRes.status === 401 || searchRes.status === 403) {
        if (statusBadge) {
          statusBadge.innerHTML = '?†Ô∏è ?∏ÏÖò ÎßåÎ£å <span style="text-decoration:underline;">(?¥Î¶≠?òÏó¨ ?∞Ïû•)</span>';
      ...
      if (existingFile) {
        ...
        if (!updateRes.ok) throw new Error('?åÏùº ??ñ¥?∞Í∏∞ ?§Ìå®');
        ...
        if (!createRes.ok) {
          ...
          throw new Error('???åÏùº ?ÖÎ°ú???§Ìå®: ' + createRes.status + ' - ' + errText);
        }
      ...
      if (statusBadge) {
        statusBadge.textContent = '?∞Í≤∞ ?ÑÎ£å (?êÎèô ?ôÍ∏∞??';
      ...
      if (statusBadge) {
        statusBadge.textContent = '?†Ô∏è ?ôÍ∏∞???§Ìå®';
        statusBadge.style.background = 'rgba(239, 68, 68, 0.15)';
        statusBadge.style.color = '#ef4444';
        statusBadge.style.borderColor = '#ef4444';
      }
    }
  }, 500); // 0.5Ï¥??îÎ∞î?¥Ïä§Î°?Ï¶âÍ∞Å?ÅÏù∏ ?ÖÎ°ú??Î∞òÏòÅ
}
...
async function performAutoRestoreAndBackup() {
  try {
    ...
    setTimeout(() => {
      alert('Íµ¨Í? ?∞Îèô Î∞??êÎèô Î≥µÏõê/Î∞±ÏóÖ???ÑÎ£å?òÏóà?µÎãà?? Î≥ÄÍ≤ΩÏÇ¨???ÅÏö©???ÑÌï¥ ?àÎ°úÍ≥†Ïπ®?©Îãà??');
      window.location.reload();
    }, 1500);

  } catch (e) {
    console.error('Auto restore/backup failed:', e);
    alert('Íµ¨Í? ?∞Îèô?Ä ?ÑÎ£å?òÏóà?ºÎÇò ?êÎèô Î≥µÏõê/Î∞±ÏóÖ Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§.');
    window.location.reload();
  }
}
...
function showSyncToast() {
  ...
  toast.innerHTML = '???§Î•∏ Í∏∞Í∏∞??Î≥ÄÍ≤ΩÏÇ¨??ù¥ ?îÎ©¥??Î∞òÏòÅ?òÏóà?µÎãà??';
...
async function autoSyncWithDrive() {
  ...
  try {
    ...
    if (existingFile.modifiedTime && existingFile.modifiedTime === lastSeenTime) {
      if (statusBadge && statusBadge.textContent !== '???êÎèô Î≥µÏõê ?ÑÎ£å (ÏµúÏã†??') {
        statusBadge.textContent = '?∞Í≤∞ ?ÑÎ£å (?êÎèô ?ôÍ∏∞??';
        statusBadge.style.background = 'rgba(16, 185, 129, 0.15)';
        statusBadge.style.color = '#10b981';
        statusBadge.style.borderColor = '#10b981';
    ...
    if (!contentRes.ok) throw new Error('Î∞±ÏóÖ ?∞Ïù¥???ΩÍ∏∞ ?§Ìå®');
    const restoreData = await contentRes.json();
    ...
    if (driveModified > localModified) {
      ...
      if (statusBadge) {
        statusBadge.textContent = '???êÎèô Î≥µÏõê ?ÑÎ£å (ÏµúÏã†??';
        statusBadge.style.background = 'rgba(16, 185, 129, 0.15)';
        statusBadge.style.color = '#10b981';
        statusBadge.style.borderColor = '#10b981';
        setTimeout(() => {
    ...
    if (statusBadge) {
      statusBadge.textContent = '?†Ô∏è ?êÎèô ?ôÍ∏∞???ïÏù∏ ?§Ìå®';
    }
  }
}
...
function updateDrilldownPanel() {
  ...
  Object.keys(state.categories).forEach(catId => {
    ...
    rateLabel.innerHTML = `?±Ï∑®?? <strong>${rate}%</strong> (${catCompleted}/${catTotal}Í∞??ÑÎ£å)`;
    ...
    if (catTasks.size === 0) {
      ...
      empty.textContent = 'Í∏∞Î°ù?????ºÏù¥ ?ÜÏäµ?àÎã§.';
      ...
      Array.from(catTasks).sort().forEach(taskText => {
        ...
        taskRateSpan.innerHTML = `?±Í≥µÎ•? <strong>${taskRate}%</strong> (${completedCount}/${occurrences.length}???ÑÎ£å)`;
        ...
        occurrences.forEach(occ => {
          ...
          statusSpan.textContent = occ.completed ? '?ÑÎ£å' : 'ÎØ∏ÏôÑÎ£?;

          tile.appendChild(dateSpan);
          tile.appendChild(statusSpan);

          tile.style.cursor = 'pointer';
          tile.title = `${formatDateKeyToMonthDay(occ.dateKey)} ?ºÏ†ï Í¥ÄÎ¶¨Î°ú ?¥Îèô`;
...
function updateCompletedDrilldownList() {
  ...
  if (sortedDates.length === 0) {
    listContainer.innerHTML = '<div class="empty-state">?ÑÎ£å?????ºÏù¥ ?ÑÏßÅ ?ÜÏäµ?àÎã§. ?ºÏ†ï???ÑÏàò??Î≥¥ÏÑ∏??</div>';
    return;
  }
  ...
  sortedDates.forEach(dk => {
    ...
    grouped[dk].forEach(todo => {
      ...
      statusSpan.textContent = '?ÑÎ£å ??;
      item.appendChild(statusSpan);

      item.title = `${formatDateKeyToMonthDay(dk)} ?ºÏ†ï Í¥ÄÎ¶¨Î°ú ?¥Îèô`;
...
function updatePendingDrilldownList() {
  ...
  if (sortedDates.length === 0) {
    listContainer.innerHTML = '<div class="empty-state">?®ÏïÑ?àÎäî ???ºÏù¥ ?ÜÏäµ?àÎã§! ?Ä?®Ìï¥??</div>';
    return;
  }
...
function updateAnalytics() {
  ...
  if (categoryBarChart) {
    ...
    Object.keys(state.categories).forEach(catId => {
      ...
      valLabel.textContent = `${rate}% (${comp}/${tot}Í∞?`;
  ...
  if (routineStatsContainer) {
    ...
    if (state.routines.length === 0) {
      routineStatsContainer.innerHTML = '<div style="color:var(--text-muted); font-size: 0.85rem; font-style: italic;">?ÑÏßÅ ?±Î°ù??Î£®Ìã¥???ÜÏäµ?àÎã§.</div>';
    } else {
      Object.keys(routineCounts).forEach(rText => {
        ...
        right.textContent = `Ï¥?${stats.completed}???ÑÎ£å`;
  ...
  if (trackerSelect) {
    ...
    if (sortedTodos.length === 0) {
      ...
      opt.value = '';
      opt.textContent = '(?±Î°ù?????ºÏù¥ ?ÜÏäµ?àÎã§)';
      trackerSelect.appendChild(opt);
    } else {
      const defaultOpt = document.createElement('option');
      defaultOpt.value = '';
      defaultOpt.textContent = '-- ???ºÏùÑ ?†ÌÉù?òÏÑ∏??--';
...
function updateSelectedTodoTracker() {
  ...
  if (!selectedText) {
    summaryText.textContent = '???ºÏùÑ ?†ÌÉù?òÏãúÎ©??ÑÎ£å Í∏∞Î°ù Î∂ÑÏÑù???òÌ??©Îãà??';
  ...
  summaryText.innerHTML = `?éØ <strong>"${selectedText}"</strong> ?±Ï∑®?? <span style="color:var(--accent-color); text-shadow:0 0 5px var(--accent-glow)">${todoRate}%</span> (Ï¥?${totalOccur}??Ï§?${completedOccur}???ÑÎ£å)`;

  historyGrid.innerHTML = '';
  if (occurrences.length === 0) {
    historyGrid.innerHTML = '<div class="empty-state">?¥Îãπ ??™©??Îß§Ïπ≠?òÎäî ?¥Î†•???ÜÏäµ?àÎã§.</div>';
    return;
  }
...
function setupEventListeners() {
  ...
  if (btnSaveNewRecord && newRecordText) {
    btnSaveNewRecord.addEventListener('click', () => {
      ...
      if (!textVal && imagesVal.length === 0 && drawingVal.length === 0 && audioVal.length === 0) {
        alert('?¥Ïö©?¥ÎÇò ?¨ÏßÑ, Í∑∏Î¶º, ?åÏÑ± Ï§??òÎÇòÎ•??ÖÎ†•??Ï£ºÏÑ∏??');
        return;
      }
  ...
  if (newRecordPhotoInput) {
    newRecordPhotoInput.addEventListener('change', (e) => {
      ...
      if (statusSpan) {
        statusSpan.textContent = '?¨ÏßÑ ?ïÏ∂ï Ï§?..';
      ...
      files.forEach(file => {
        compressAndSaveImage(file, (dataUrl) => {
          ...
          if (processed === files.length) {
            renderDiary();
            if (statusSpan) {
              statusSpan.textContent = 'Í∏∞Î°ù Ï∂îÍ? Ï§?..';
  ...
  if (btnTodoSave) {
    btnTodoSave.addEventListener('click', () => {
      ...
      const newVideo = [...todoEditDraftVideo];
  ...
  if (btnTodoDelete) {
    btnTodoDelete.addEventListener('click', () => {
      if (editingTodoId && confirm('?????ºÏùÑ ??†ú?òÏãúÍ≤†Ïäµ?àÍπå?')) {
        const dateKey = state.selectedDate;
        const todo = state.todos[dateKey].find(t => t.id === editingTodoId);
        if (todo) {
  ...
  if (btnTimeConfirm) {
    btnTimeConfirm.addEventListener('click', (e) => {
      ...
      if (hourVal && minVal) {
        ...
        currentSelectedTime = '';
        if (btnTimeTrigger) {
          btnTimeTrigger.textContent = '???úÍ∞Ñ ?§Ï†ï';
        }
      }
      if (timeDropdown) timeDropdown.classList.add('hidden');
    });
  ...
  const saveClientId = () => {
    if (gdriveClientIdInput) {
      ...
      alert('?îë Íµ¨Í? Client IDÍ∞Ä ?àÏ†Ñ?òÍ≤å ?±Î°ù?òÏóà?µÎãà??');
    }
  };
  ...
  if (gdriveLoginBtn) {
    gdriveLoginBtn.addEventListener('click', () => {
      ...
      if (!clientId) {
        alert('Íµ¨Í? ?úÎùº?¥Î∏å ?∞Îèô??ÏßÑÌñâ?òÎ†§Î©?Î®ºÏ? Î∞úÍ∏âÎ∞õÏúº??"Íµ¨Í? Client ID"Î•??ÑÎûò ?ÅÏûê???ÖÎ†•?¥Ï£º?îÏïº ?©Îãà??');
        return;
      }

      if (typeof google === 'undefined' || !google.accounts || !google.accounts.oauth2) {
        alert('Íµ¨Í? Î°úÍ∑∏???ºÏù¥Î∏åÎü¨Î¶¨Í? Î°úÎìú?òÏ? ?äÏïò?µÎãà??\n\n?∏ÌÑ∞???∞Í≤∞???ïÏù∏?òÏãúÍ±∞ÎÇò, Î∏åÎùº?∞Ï???Í¥ëÍ≥† Ï∞®Îã® ?ÑÎ°úÍ∑∏Îû®(AdBlock, Brave Shield ????Íµ¨Í? ?∏Ï¶ù ?§ÌÅ¨Î¶ΩÌä∏Î•?Ï∞®Îã®?òÍ≥† ?àÎäîÏßÄ ?ïÏù∏?????àÎ°úÍ≥†Ïπ®?òÏó¨ ?§Ïãú ?úÎèÑ??Ï£ºÏÑ∏??');
        return;
      }
      ...
      try {
        gdriveTokenClient = google.accounts.oauth2.initTokenClient({
          ...
          callback: (tokenResponse) => {
            if (tokenResponse.error !== undefined) {
              alert('Íµ¨Í? ?∏Ï¶ù???§Ìå®?àÏäµ?àÎã§: ' + tokenResponse.error);
              return;
            }
            gdriveAccessToken = tokenResponse.access_token;
            const expiryTime = Date.now() + (tokenResponse.expires_in * 1000);
            ...
            const info = document.getElementById('gdrive-user-info');
            if (info) {
              info.textContent = 'Íµ¨Í? ?∞Îèô ?úÏÑ±??(?§ÏãúÍ∞??êÎèô ?ôÍ∏∞??';
            }
        ...
        console.error(err);
        alert('Íµ¨Í? Î°úÍ∑∏???¥Îùº?¥Ïñ∏???ùÏÑ± ?§Ìå®: ' + err.message + '\nClient ID ?ïÏãù???¨Î∞îÎ•∏Ï? ?¨Ï∞® ?ïÏù∏?¥Ï£º?∏Ïöî.');
      }
    });
  }
  ...
  if (btnLogoutGDrive) {
    btnLogoutGDrive.addEventListener('click', () => {
      ...
      const info = document.getElementById('gdrive-user-info');
      if (info) info.textContent = 'Íµ¨Í? Î°úÍ∑∏?????¥Îùº?∞Îìú ?§ÏãúÍ∞??ôÍ∏∞??;

      alert('Íµ¨Í? Í≥ÑÏ†ï ?∞Îèô???ïÏÉÅ?ÅÏúºÎ°??¥Ï†ú?àÏäµ?àÎã§.');
    });
  }
  ...
  if (gdriveBackupBtn) {
    gdriveBackupBtn.addEventListener('click', async () => {
      if (!gdriveAccessToken) {
        alert('Íµ¨Í? ?∞Îèô ÎßåÎ£å ?êÎäî ÎØ∏Ïó∞Í≤??ÅÌÉú?ÖÎãà?? Íµ¨Í? Î°úÍ∑∏??Î≤ÑÌäº???§Ïãú ?åÎü¨Ï£ºÏÑ∏??');
        return;
      }

      gdriveBackupBtn.disabled = true;
      gdriveBackupBtn.textContent = '?ì§ Î∞±ÏóÖ Ï§?..';
      ...
      try {
        ...
        if (searchRes.status === 401 || searchRes.status === 403) {
          alert('Íµ¨Í? Î°úÍ∑∏???∏ÏÖò??ÎßåÎ£å?òÏóà?µÎãà?? ?∏Ï¶ù Ï∞ΩÏù¥ ?πÎãà??');
          const loginBtn = document.getElementById('btn-gdrive-login');
          if (loginBtn) loginBtn.click();
          throw new Error('?∏Ï¶ù ÎßåÎ£å (?¨Î°úÍ∑∏Ïù∏ ÏßÑÌñâ)');
        }
        const searchData = await searchRes.json();
        const existingFile = searchData.files && searchData.files[0];
        ...
        alert('Íµ¨Í? ?úÎùº?¥Î∏å Î∞±ÏóÖ???ÑÎ£å?òÏóà?µÎãà?? (neon_planner_backup.json ?åÏùºÎ°??Ä?•Îê®)');
      } catch (err) {
        console.error(err);
        alert('Î∞±ÏóÖ ?ÖÎ°ú??Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§: ' + err.message);
      } finally {
        gdriveBackupBtn.disabled = false;
        gdriveBackupBtn.textContent = '?ì§ ?úÎùº?¥Î∏å Î∞±ÏóÖ';
      }
    });
  }
  ...
  if (gdriveRestoreBtn) {
    gdriveRestoreBtn.addEventListener('click', async () => {
      ...
      if (!confirm('?ïÎßêÎ°?Íµ¨Í? ?úÎùº?¥Î∏å?êÏÑú Î∞±ÏóÖ ?∞Ïù¥?∞Î? Î∞õÏïÑ?Ä ??ñ¥?åÏö∞?úÍ≤†?µÎãàÍπ?\n?ÑÏû¨ Î°úÏª¨ ?∞Ïù¥?∞Îäî Î™®Îëê ?†Ïã§?©Îãà??')) {
        return;
      }

      gdriveRestoreBtn.disabled = true;
      gdriveRestoreBtn.textContent = '?ì• Î≥µÏõê Ï§?..';
      ...
      try {
        ...
        if (!existingFile) {
          alert('Íµ¨Í? ?úÎùº?¥Î∏å ?¥Ïóê Î∞±ÏóÖ???åÏùº(neon_planner_backup.json)??Î∞úÍ≤¨?????ÜÏäµ?àÎã§.');
          return;
        }
        ...
        if (!contentRes.ok) throw new Error('Î∞±ÏóÖ ?∞Ïù¥???åÏùº ?ΩÍ∏∞ ?§Ìå®');
        const restoreData = await contentRes.json();
        ...
        alert('Íµ¨Í? ?úÎùº?¥Î∏å Î∞±ÏóÖ ?∞Ïù¥??Î≥µÏõê???±Í≥µ?àÏäµ?àÎã§! Î≥ÄÍ≤ΩÏÇ¨???ÅÏö©???ÑÌï¥ ?îÎ©¥???àÎ°úÍ≥†Ïπ®?©Îãà??');
        window.location.reload();
      } catch (err) {
        console.error(err);
        alert('Î≥µÏõê ?§Ïö¥Î°úÎìú Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§: ' + err.message);
      } finally {
        gdriveRestoreBtn.disabled = false;
        gdriveRestoreBtn.textContent = '?ì• ?úÎùº?¥Î∏å Î≥µÏõê';
      }
    });
  }
...
function openTodoEditModal(todoId) {
  ...
  todoEditDraftVideo = todo.memoVideo ? JSON.parse(JSON.stringify(todo.memoVideo)) : [];
  ...
  if (catsContainer) {
    ...
    noneBtn.innerHTML = `<div class="todo-modal-cat-dot" style="background-color: #888;"></div>?ÜÏùå`;
...
function handleDeleteCategory(catId) {
  if (!confirm(`'${state.categories[catId].label}' Ïπ¥ÌÖåÍ≥†Î¶¨Î•???†ú?òÏãúÍ≤†Ïäµ?àÍπå?\nÍ∏∞Ï°¥ ???ºÎì§??Í∏Ä?®Îäî ?†Ï??òÎ©∞, ?¨Î†•???ÑÌä∏???∞ÏÉâ?ºÎ°ú Î≥ÄÍ≤ΩÎê©?àÎã§.`)) {
    return;
  }
...
function renderSearchResultsSection() {
  ...
  // 1. Search Tabs
  const tabs = [
    { name: '?¨Î†•', id: 'btn-toggle-calendar' },
    { name: '?†Ïùº', id: 'btn-toggle-todos' },
    { name: 'Í∏∞Î°ù', id: 'btn-toggle-records' },
    { name: '?Ä?ÑÎùº??, id: 'btn-toggle-timeline' },
    { name: '?îÎç∞??, id: 'btn-toggle-ddays' },
    { name: 'Î∂ÑÏÑù', id: 'btn-toggle-analytics' },
    { name: '?§Ï†ï', id: 'btn-toggle-control-panel' }
  ];
  ...
  if (results.length === 0) {
    ...
    empty.textContent = 'Í≤Ä??Í≤∞Í≥ºÍ∞Ä ?ÜÏäµ?àÎã§.';
...
function updateUI() {
  ...
  if (logoText) {
    let titleStr = state.appTitle || '?åÎûò??;
    titleStr = titleStr.replace(/?ìÅ/g, '').trim(); // Remove any folder emoji user might have added
    logoText.textContent = titleStr;
  }

  if (btnToggleSearch) {
    btnToggleSearch.innerHTML = `${state.tabIcons.search || '?îç'} <span class="btn-text">${highlightMarkup('Í≤Ä??, state.searchQuery)}</span>`;
  }
  if (btnToggleCalendar) {
    btnToggleCalendar.innerHTML = `${state.tabIcons.calendar || '?ìÖ'} <span class="btn-text">${highlightMarkup('?¨Î†•', state.searchQuery)}</span>`;
  }
  if (btnToggleTodos) {
    btnToggleTodos.innerHTML = `${state.tabIcons.todos || '?éØ'} <span class="btn-text">${highlightMarkup('?†Ïùº', state.searchQuery)}</span>`;
  }
  if (btnToggleRecords) {
    btnToggleRecords.innerHTML = `${state.tabIcons.records || '?ìù'} <span class="btn-text">${highlightMarkup('Í∏∞Î°ù', state.searchQuery)}</span>`;
  }
  if (btnToggleRoutines) {
    btnToggleRoutines.innerHTML = `${state.tabIcons.routines || '?îÑ'} <span class="btn-text">${highlightMarkup('Î£®Ìã¥', state.searchQuery)}</span>`;
  }
  if (btnToggleTimeline) {
    btnToggleTimeline.innerHTML = `${state.tabIcons.timeline || '??} <span class="btn-text">${highlightMarkup('?Ä?ÑÎùº??, state.searchQuery)}</span>`;
  }
  const btnToggleDdays = document.getElementById('btn-toggle-ddays');
  if (btnToggleDdays) {
    btnToggleDdays.innerHTML = `${state.tabIcons.ddays || '?éâ'} <span class="btn-text">${highlightMarkup('?îÎç∞??, state.searchQuery)}</span>`;
  }
  if (btnToggleAnalytics) {
    btnToggleAnalytics.innerHTML = `${state.tabIcons.analytics || '?ìä'} <span class="btn-text">${highlightMarkup('Î∂ÑÏÑù', state.searchQuery)}</span>`;
  }
  if (btnToggleControlPanel) {
    btnToggleControlPanel.innerHTML = `${state.tabIcons.settings || '?ôÔ∏è'} <span class="btn-text">${highlightMarkup('?§Ï†ï', state.searchQuery)}</span>`;
  }
...
function renderCategoryFilterTabs() {
  ...
  routineTab.title = 'Î£®Ìã¥(Îß§Ïùº Î∞òÎ≥µ) ???ºÎßå Î≥¥Í∏∞ (?¥Î¶≠ ???†Í?)';
  ...
  routineLabelSpan.innerHTML = '?îÑ Î£®Ìã¥';
  ...
  Object.keys(state.categories).forEach(catId => {
    ...
    tab.title = `${cat.label} ??ùò ???ºÎßå Î≥¥Í∏∞ (?¥Î¶≠ ???†Í?)`;
...
function renderCategorySelector() {
  ...
  Object.keys(state.categories).forEach(catId => {
    ...
    if (catId !== 'other') {
      ...
      editBtn.innerHTML = '?èÔ∏è';
      editBtn.title = 'Ïπ¥ÌÖåÍ≥†Î¶¨ ?òÏ†ï';
      ...
      option.title = '?îÎ∏î?¥Î¶≠?òÏó¨ Ïπ¥ÌÖåÍ≥†Î¶¨ ?òÏ†ï (Î™®Î∞î?? Í∏∏Í≤å ?ÑÎ•¥Í∏?';
  ...
  addTrigger.textContent = '+ Ï∂îÍ?';
  addTrigger.title = '??Ïπ¥ÌÖåÍ≥†Î¶¨ Ï∂îÍ?';
...
function renderDiary() {
  ...
  if (records.length === 0 && state.editingRecordId !== 'new') {
    ...
    emptyState.innerHTML = query !== '' 
      ? `<p>Í≤Ä??Í≤∞Í≥ºÍ∞Ä ?ÜÏäµ?àÎã§.</p>` 
      : `<p>?ÑÏßÅ Í∏∞Î°ù???¥Ïö©???ÜÏäµ?àÎã§. ?§Îäò ?òÎ£®Î•?Í∏∞Î°ù??Î≥¥ÏÑ∏??</p>`;
    ...
    records.forEach(record => {
      ...
      if (query !== '') {
        ...
        dateHeader.textContent = `?ìÖ ${formatDateKeyToMonthDay(record.dateKey)}`;
        dateHeader.title = `${record.dateKey}Î°??¥Îèô`;
      ...
      if (state.editingRecordId === record.id) {
        ...
        textarea.placeholder = 'Í∏∞Î°ù ?¥Ïö©???òÏ†ï??Î≥¥ÏÑ∏??..';
        ...
        dictateBtn.title = '?åÏÑ± ?πÏùå Î∞??çÏä§??Î≥Ä??STT)';
        dictateBtn.style.top = '8px';
        dictateBtn.innerHTML = '?éôÔ∏?;
        ...
        label.appendChild(document.createTextNode('?ì∑ ?¨ÏßÑ ?†ÌÉù (?¨Îü¨??Í∞Ä??'));
        mediaRow.appendChild(label);

        const statusSpan = document.createElement('span');
        statusSpan.className = 'diary-save-status';
        statusSpan.textContent = '?òÏ†ï Ï§?..';
        ...
        drawingLabel.innerHTML = '<span class="upload-icon">?é®</span> ?êÍ∑∏Î¶?Í∑∏Î¶¨Í∏?(?¥Í∏∞/?´Í∏∞)';
        ...
        saveBtn.addEventListener('click', () => {
          ...
          record.video = [...(state.diaryDraftVideo || [])];
          saveDiaries();
          state.editingRecordId = null;
          state.diaryDraftImages = [];
          state.diaryDraftDrawing = [];
      ...
      state.diaryDraftVideo = [];
        ...
        if (record.drawing && record.drawing.length > 0) {
          ...
          drawingToggleBtn.innerHTML = '?ñºÔ∏?Ï≤®Î???Í∑∏Î¶º Î≥¥Í∏∞ (?¥Î¶≠?òÏó¨ ?ºÏπòÍ∏?';
          ...
          viewDrawingContainer.title = '?¥Î¶≠?òÏó¨ ?ºÍ∏∞ ?òÏ†ï?òÍ∏∞';
          const openEdit = (e) => {
            e.stopPropagation();
            e.preventDefault();
        ...
        if (record.video && record.video.length > 0) {
          const videoViewContainer = document.createElement('div');
          videoViewContainer.className = 'video-previews-container';
          videoViewContainer.style.marginTop = '8px';
          card.appendChild(videoViewContainer);
          setTimeout(() => {
            renderVideoPreviews(videoViewContainer.id || (videoViewContainer.id = `view-video-${record.id}`), record.video, null);
          }, 0);
        }
        ...
        editBtn.addEventListener('click', () => {
          ...
          state.diaryDraftVideo = record.video ? JSON.parse(JSON.stringify(record.video)) : [];
          renderDiary();
        });
        ...
        exportBtn.innerHTML = '?íæ ?Ä??;
        exportBtn.title = '???ºÍ∏∞Î•??åÏùº(HTML)Î°??Ä?•Ìï©?àÎã§';
        exportBtn.addEventListener('click', () => {
          let htmlContent = `<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
<title>?ºÍ∏∞ Í∏∞Î°ù - ${dateKey}</title>
...
`;
          ...
          if (record.drawing && record.drawing.length > 0) {
             ...
             if (canvas) {
                ...
                htmlContent += `<h3>?é® ?êÍ∑∏Î¶?/h3><img class="drawing-img" src="${tempCanvas.toDataURL('image/png')}">`;
             }
          }
...
function renderTimeline() {
  ...
  if (sortedDates.length === 0) {
    ...
    placeholder.textContent = '?ÑÏßÅ Í∏∞Î°ù?????ºÏù¥???ºÍ∏∞Í∞Ä ?ÜÏäµ?àÎã§. ???ºÏùÑ ?±Î°ù?òÍ±∞???ºÍ∏∞Î•??®Î≥¥?∏Ïöî!';
    timelineList.appendChild(placeholder);
    return;
  }

  const weekdays = ['?ºÏöî??, '?îÏöî??, '?îÏöî??, '?òÏöî??, 'Î™©Ïöî??, 'Í∏àÏöî??, '?†Ïöî??];
  ...
  sortedDates.forEach(dateKey => {
    ...
    if (todos.length === 0) {
      ...
      emptyMsg.textContent = '?±Î°ù?????ºÏù¥ ?ÜÏäµ?àÎã§.';
      ...
      sortedTodos.forEach(todo => {
        ...
        delBtn.addEventListener('click', (e) => {
          e.stopPropagation();
          if (confirm('?????ºÏùÑ ??†ú?òÏãúÍ≤†Ïäµ?àÍπå?')) {
    ...
    diaryTitle.innerHTML = '?ìù ?§Îäò??Í∏∞Î°ù';
    ...
    if (records.length === 0) {
      const emptyMsg = document.createElement('div');
      emptyMsg.style = 'color: var(--text-muted); font-size: 0.8rem; font-style: italic;';
      emptyMsg.textContent = '?ëÏÑ±??Í∏∞Î°ù???ÜÏäµ?àÎã§.';
...
function toggleCategoryForm(show) {
  ...
  if (show) {
    ...
    if (state.editingCategoryId) {
      // EDIT MODE
      if (headerSpan) headerSpan.textContent = 'Ïπ¥ÌÖåÍ≥†Î¶¨ ?òÏ†ï';
      if (saveBtn) saveBtn.textContent = '?òÏ†ï';
      ...
      // ADD MODE
      if (headerSpan) headerSpan.textContent = '??Ïπ¥ÌÖåÍ≥†Î¶¨ Ï∂îÍ?';
      if (saveBtn) saveBtn.textContent = 'Ï∂îÍ?';
...
function updateCategoryPreview() {
  ...
  const label = nameInput.value.trim() || 'ÎØ∏Î¶¨Î≥¥Í∏∞';
...
function handleSaveCategory() {
  ...
  if (!label) {
    alert('Ïπ¥ÌÖåÍ≥†Î¶¨ ?¥Î¶Ñ???ÖÎ†•?¥Ï£º?∏Ïöî.');
    nameInput.focus();
    return;
  }
  ...
  if (isDuplicate) {
    alert('?¥Î? Ï°¥Ïû¨?òÎäî Ïπ¥ÌÖåÍ≥†Î¶¨ ?¥Î¶Ñ?ÖÎãà??');
    nameInput.focus();
    return;
  }
...
function createCell(day, dateKey, isOtherMonth = false, isToday = false) {
  ...
  if (state.ddays && state.ddays.length > 0) {
    ...
    if (activeDdays.length > 0) {
      ...
      ddayPill.textContent = isAnniv ? 'Í∏∞ÎÖê?? : ddayInfo.label;
...
function applyCopyModeBanner() {
  ...
  if (state.copyingTodoId) {
    ...
    if (spanText) {
      spanText.innerHTML = `?éØ ?¨Î†•?êÏÑú ?†ÏßúÎ•??¥Î¶≠?òÎ©¥ <strong>"${todoText}"</strong> ???ºÏù¥ ?¥Îãπ ?†Ïßú??Î≥µÏÇ¨?©Îãà??`;
    }
  } else {
...
function calculateDday(targetDateStr, type) {
  ...
  if (type === 'anniversary') {
    ...
    if (diffDays === 0) {
      return { label: '?§Îäò Í∏∞ÎÖê??', sub: `${yearsCount}Ï£ºÎÖÑ` };
    } else {
      return { label: `D-${diffDays}`, sub: `${yearsCount}Ï£ºÎÖÑ (${nextOccurrence.getMonth() + 1}/${nextOccurrence.getDate()})` };
    }
  } else {
    const diffMs = target - today;
    const diffDays = Math.ceil(diffMs / (1000 * 60 * 60 * 24));
    if (diffDays > 0) {
      return { label: `D-${diffDays}`, sub: '?®Ïùå' };
    } else if (diffDays === 0) {
      return { label: 'D-Day', sub: '?§Îäò!' };
    } else {
      return { label: `D+${Math.abs(diffDays)}`, sub: 'ÏßÄ?? };
    }
  }
}
...
function renderDdays() {
  ...
  if (state.ddays.length === 0) {
    ...
    emptyState.textContent = '?±Î°ù???îÎç∞?¥ÎÇò Í∏∞ÎÖê?ºÏù¥ ?ÜÏäµ?àÎã§. ?§Î•∏Ï™??ÑÏùò "?îÎç∞??Ï∂îÍ?" Î≤ÑÌäº???åÎü¨ ?±Î°ù??Î≥¥ÏÑ∏??';
    listContainer.appendChild(emptyState);
    return;
  }
  ...
  sortedDdays.forEach(dday => {
    ...
    delBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      if (confirm(`"${dday.title}" ?îÎç∞?¥Î? ?ïÎßêÎ°???†ú?òÏãúÍ≤†Ïäµ?àÍπå?`)) {
        deleteDday(dday.id);
      }
    });
...
function openDdayModal(ddayId = null) {
  ...
  if (ddayId) {
    modalTitle.textContent = '?éâ ?îÎç∞???òÏ†ï';
    ...
    modalTitle.textContent = '?éâ ?îÎç∞??Ï∂îÍ?';
  }
...
function saveDdayData() {
  ...
  if (!title) {
    alert('?îÎç∞???úÎ™©???ÖÎ†•??Ï£ºÏÑ∏??');
    return;
  }
  if (!date) {
    alert('Î™©Ìëú ?†ÏßúÎ•??†ÌÉù??Ï£ºÏÑ∏??');
    return;
  }
...
function renderRoutinesPanel() {
  ...
  if (!state.routines || state.routines.length === 0) {
    container.innerHTML = '<div style="text-align: center; color: var(--text-secondary); padding: 20px; font-size: 0.9rem;">?±Î°ù??Î£®Ìã¥???ÜÏäµ?àÎã§. ?àÎ°ú??Î£®Ìã¥??Ï∂îÍ???Î≥¥ÏÑ∏??</div>';
    return;
  }
  ...
  state.routines.forEach(routine => {
    ...
    text.title = '?¥Î¶≠/?îÎ∏î?¥Î¶≠/Í∏∏Í≤å ?ÑÎ•¥Í∏? Î£®Ìã¥ ?òÏ†ï';
    ...
    if (routine.startDate || routine.endDate) {
      ...
      dateMeta.textContent = `?óìÔ∏?${routine.startDate || 'Í≥ÑÏÜç'} ~ ${routine.endDate || 'Í≥ÑÏÜç'}`;
...
function deleteRoutine(routineId) {
  if (confirm('??Î£®Ìã¥????†ú?òÏãúÍ≤†Ïäµ?àÍπå?\\n(?¥Î? ?±Î°ù??Í≥ºÍ±∞/?§Îäò?????ºÏ? ??†ú?òÏ? ?äÏäµ?àÎã§)')) {
    pushToHistory();
...
function openRoutineEditModal(routineId) {
  ...
  if (catsContainer) {
    ...
    otherBtn.innerHTML = `<div class="todo-modal-cat-dot" style="background-color: #888;"></div>?ÜÏùå/Í∏∞Î≥∏`;
...
// Parse natural language date (e.g. 7??0?? 7/20) and time (e.g. ?§Ï†Ñ 1??20Î∂? ?êÏ†ï 30Î∂? 14:30)
function parseNaturalLanguageTodo(inputText) {
  ...
  // 1. Relative Dates (?§Îäò, ?¥Ïùº, Î™®Î†à, ?¥Ï†ú, Í∑∏Ï†ú)
  const relativeDateRegex = /(Í∑∏Ï†ú|Í∑∏Ï?Íª??¥Ï†ú|?§Îäò|?¥Ïùº|Î™®Î†à|Í∏Ä??/i;
  const relMatch = text.match(relativeDateRegex);
  if (relMatch) {
    const keyword = relMatch[1];
    const todayDate = new Date();
    let offset = 0;
    if (keyword === 'Í∑∏Ï†ú' || keyword === 'Í∑∏Ï?Íª?) offset = -2;
    else if (keyword === '?¥Ï†ú') offset = -1;
    else if (keyword === '?§Îäò') offset = 0;
    else if (keyword === '?¥Ïùº') offset = 1;
    else if (keyword === 'Î™®Î†à') offset = 2;
    else if (keyword === 'Í∏Ä??) offset = 3;
  ...
  // 2. Regex for Time:
  // A. "?êÏ†ï 20Î∂?, "?êÏ†ï", "?ïÏò§ 15Î∂?, "?ïÏò§"
  const midnightNoonRegex = /(?êÏ†ï|?ïÏò§)(?:\s*(\d{1,2})\s*Î∂??/i;
  let timeMatch = text.match(midnightNoonRegex);
  if (timeMatch) {
    const keyword = timeMatch[1];
    const minStr = timeMatch[2] || '00';
    const hour = keyword === '?êÏ†ï' ? 0 : 12;
  ...
  // B. "?§Ï†Ñ 1??20Î∂?, "?§ÌõÑ 12??5Î∂?, "?§Ï†Ñ 1??, "?§ÌõÑ 3??
  if (!parsedTime) {
    const ampmRegex = /(?§Ï†Ñ|?§ÌõÑ)\s*(\d{1,2})\s*???:\s*(\d{1,2})\s*Î∂??/i;
  ...
  // 3. Importance keywords (‚≠? ?? [Ï§ëÏöî], Ï§ëÏöî:)
  let isImportant = false;
  const importantRegex = /(?:^|\s)(?:‚≠???\[Ï§ëÏöî\]|Ï§ëÏöî:)\s*/i;
...
// Format HH:MM 24h string into Korean AM/PM Hour/Minute with Midnight (?êÏ†ï) & Noon (?ïÏò§) conversions
function formatTimeKorean(timeStr) {
  ...
  if (hour24 === 0) {
    if (minVal === '00') return '?êÏ†ï';
    return `?êÏ†ï ${parseInt(minVal, 10)}Î∂?;
  }
  if (hour24 === 12) {
    if (minVal === '00') return '?ïÏò§';
    return `?ïÏò§ ${parseInt(minVal, 10)}Î∂?;
  }

  const ampm = hour24 >= 12 ? '?§ÌõÑ' : '?§Ï†Ñ';
  let hour12 = hour24 % 12;
  if (hour12 === 0) hour12 = 12;
  return `${ampm} ${hour12}??${minVal}Î∂?;
}
...
function renderTodos() {
  ...
  if (state.selectedDate === todayStr) {
    selectedDateDisplay.textContent = `${formattedText} - ?§Îäò`;
  } else {
    selectedDateDisplay.textContent = formattedText;
  }

  if (todoInputField) {
    todoInputField.placeholder = `${dateObj.getMonth() + 1}??${dateObj.getDate()}?ºÏùò ?àÎ°ú?????ºÏùÑ ?ÖÎ†•??Î≥¥ÏÑ∏??..`;
  }
  ...
  if (dayTodos.length === 0) {
    ...
    emptyDiv.textContent = query !== '' ? 'Í≤Ä??Í≤∞Í≥ºÍ∞Ä ?ÜÏäµ?àÎã§.' : '???ºÏù¥ ?ÜÏäµ?àÎã§. ?àÎ°ú?????ºÏùÑ Ï∂îÍ???Î≥¥ÏÑ∏??';
  ...
  if (filteredTodos.length === 0) {
    if (filterCat === 'routine') {
      ...
      emptyDiv.innerHTML = `
        <div style="display: flex; align-items: center; gap: 8px;">
          <span style="font-size: 1.1rem;">?îÑ</span>
          <span>?†ÌÉù???†Ïßú???±Î°ù??<strong>Î£®Ìã¥</strong> ???ºÏù¥ ?ÜÏäµ?àÎã§.</span>
        </div>
    ...
    const activeCatLabel = activeCatObj ? activeCatObj.label : '?†ÌÉù??Ïπ¥ÌÖåÍ≥†Î¶¨';
    const emptyDiv = document.createElement('div');
    emptyDiv.className = 'todo-filter-empty-state';
    emptyDiv.innerHTML = `
      <div style="display: flex; align-items: center; gap: 8px;">
        <span style="font-size: 1.1rem;">?éØ</span>
        <span><strong>'${escapeHtml(activeCatLabel)}'</strong> ??óê ?±Î°ù?????ºÏù¥ ?ÜÏäµ?àÎã§.</span>
      </div>
      <div class="empty-actions">
        <button type="button" class="empty-btn primary" id="btn-quick-add-to-cat">
          ??'${escapeHtml(activeCatLabel)}' ????Ï∂îÍ??òÍ∏∞
        </button>
        <button type="button" class="empty-btn" id="btn-show-all-todos-filter">
          ?ìã ?ÑÏ≤¥ ????Î≥¥Í∏∞
        </button>
  ...
  sortedTodos.forEach(todo => {
    ...
    textSpan.title = '?¥Î¶≠: ?ºÏ†ï Í≥µÏú†/Ï∂îÍ? | ?îÎ∏î?¥Î¶≠: ?òÏ†ï | Íæ??ÑÎ•¥Í∏? ?úÏÑú ?¥Îèô';
    ...
    if (todo.isImportant) {
      ...
      importantBadge.innerHTML = '‚≠?Ï§ëÏöî';
    ...
    if (todo.isRoutine) {
      ...
      routineBadge.textContent = 'Î£®Ìã¥';
      routineBadge.style.cursor = 'pointer';
      routineBadge.title = '?¥Î¶≠ ??Î£®Ìã¥ Î™©Î°ùÎß?Î≥¥Í∏∞';
    ...
    badge.title = `?¥Î¶≠ ??'${cat.label}' Ïπ¥ÌÖåÍ≥†Î¶¨Îß?Î≥¥Í∏∞`;
    ...
    if (state.searchQuery.trim() !== '') {
      ...
      datePill.title = `${todo.dateKey}Î°??¥Îèô`;
    ...
    if (todo.memoDrawing && todo.memoDrawing.length > 0) {
      const drawingToggleBtn = document.createElement('div');
      drawingToggleBtn.className = 'record-drawing-toggle';
      drawingToggleBtn.innerHTML = '?ñºÔ∏?Ï≤®Î???Í∑∏Î¶º Î≥¥Í∏∞/?òÏ†ï (?¥Î¶≠)';
      ...
      drawingToggleBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        openGlobalDrawingModal(todo.memoDrawing, (newData) => {
          todo.memoDrawing = newData;
          pushToHistory();
          saveTodos();
          updateUI();
        });
      ...
      viewDrawingContainer.style.cursor = 'pointer';
      viewDrawingContainer.title = '?¥Î¶≠?òÏó¨ ?????òÏ†ï?òÍ∏∞';
      const openEdit = (e) => {
        e.stopPropagation();
        e.preventDefault();
        openTodoEditModal(todo.id);
    ...
    starBtn.title = todo.isImportant ? 'Ï§ëÏöî ?úÏãú ?¥Ï†ú' : 'Ï§ëÏöî ?úÏãú (?ÅÎã® Í≥†Ï†ï)';
    starBtn.ariaLabel = todo.isImportant ? 'Ï§ëÏöî ?úÏãú ?¥Ï†ú' : 'Ï§ëÏöî ?úÏãú';
    ...
    if (state.selectedTodoIdForDates === todo.id) {
      ...
      if (state.copyingTodoId === todo.id) {
        ...
        copyBtn.innerHTML = '?éØ ?¨Î†•?êÏÑú Î≥µÏÇ¨???†Ïßú ?†ÌÉù Ï§?.. (?ÑÎ£å?òÎ†§Î©??¥Î¶≠)';
      } else {
        copyBtn.innerHTML = '???§Î•∏ ?†Ïßú??Ï∂îÍ??òÍ∏∞';
      }
  ...
  if (btnLocalBackup) {
    btnLocalBackup.addEventListener('click', () => {
      try {
        ...
        alert("Î∞±ÏóÖ ?åÏùº ?ùÏÑ± Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§: " + err.message);
      }
    });
  }
    ...
    localRestoreInput.addEventListener('change', (e) => {
      ...
      reader.onload = function(event) {
        try {
          ...
          if (!confirm('?†ÌÉù??Î∞±ÏóÖ ?åÏùºÎ°?Î™®Îì† ?∞Ïù¥?∞Î? Î≥µÍµ¨?òÏãúÍ≤†Ïäµ?àÍπå?\n?ÑÏû¨ ?∞Ïù¥?∞Îäî Î™®Îëê ??ñ¥?åÏõåÏßëÎãà??')) {
          ...
          alert('Î°úÏª¨ Î∞±ÏóÖ Î≥µÏõê???ÑÎ£å?òÏóà?µÎãà?? ?îÎ©¥???àÎ°úÍ≥†Ïπ®?©Îãà??');
          location.reload();
        } catch (err) {
          alert('Î∞±ÏóÖ ?åÏùº???ΩÎäî Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§. ?¨Î∞îÎ•??åÏùº?∏Ï? ?ïÏù∏?¥Ï£º?∏Ïöî.\n' + err.message);
        }
...
window.addEventListener('online', () => {
  if (localStorage.getItem('neon_planner_gdrive_connected') === 'true' && typeof autoSyncWithDrive === 'function') {
    const badge = document.getElementById('gdrive-status-badge');
    if (badge) {
      badge.textContent = '?åê ?∏ÌÑ∞???¨Ïó∞Í≤∞Îê®...';
    }
...
function initVoiceAssistant() {
  ...
  const startListening = (targetInput) => {
    ...
    if (targetInput) {
      ...
      if (voiceStatusText) voiceStatusText.textContent = '?£Í≥† ?àÏäµ?àÎã§...';
    }
  };
  ...
  recognition.onerror = (event) => {
    ...
    if (!currentTargetInput && voiceStatusText) {
      voiceStatusText.textContent = '?§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§. ?§Ïãú ?úÎèÑ?¥Ï£º?∏Ïöî.';
...
function handleVoiceCommand(transcript, recognition, stopListeningUI) {
  ...
  if (t.includes('Ï¢ÖÎ£å') || t.includes('Í∑∏Îßå') || t.includes('?´ÏïÑ') || t.includes('?ùÎÇ¥')) {
    if (recognition) recognition.stop();
    if (stopListeningUI) stopListeningUI();
    return;
  }
  
  // Navigation commands (No return, so it can chain with 'Ï∂îÍ?')
  if (t.includes('??????) || t.includes('?†Ïùº ??) || t.includes('?†Ïùº ?¥Ïñ¥') || t.includes('?????¥Ïñ¥') || t.includes('?¨Îëê')) {
    const btn = document.getElementById('btn-toggle-todos');
    if (btn && !btn.classList.contains('active-view')) btn.click();
  } else if (t.includes('Î∂ÑÏÑù') || t.includes('?µÍ≥Ñ')) {
    const btn = document.getElementById('btn-toggle-analytics');
    if (btn && !btn.classList.contains('active-view')) btn.click();
  } else if (t.includes('?¨Î†•') || t.includes('Ï∫òÎ¶∞??)) {
    const btn = document.getElementById('btn-toggle-calendar');
    if (btn && !btn.classList.contains('active-view')) btn.click();
  } else if (t.includes('Í∏∞Î°ù') || t.includes('?ºÍ∏∞')) {
    const btn = document.getElementById('btn-toggle-records');
    if (btn && !btn.classList.contains('active-view')) btn.click();
  } else if (t.includes('Î£®Ìã¥')) {
    const btn = document.getElementById('btn-toggle-routines');
    if (btn && !btn.classList.contains('active-view')) btn.click();
  } else if (t.includes('?§Ï†ï')) {
    const btn = document.getElementById('btn-toggle-control-panel');
    if (btn && !btn.classList.contains('active-view')) btn.click();
  }
  
  // Adding Command (Todo, Routine, Record, D-day)
  if (t.includes('Ï∂îÍ?') || t.includes('?£Ïñ¥') || t.includes('?Ä??)) {
    const textToParse = t.replace(/??s*?ºÏóê?/gi, '')
                         .replace(/?ºÏ†ï??/gi, '')
                         .replace(/Î£®Ìã¥\s*Í¥ÄÎ¶¨Ïóê?/gi, '')
                         .replace(/Î£®ÌåÖ\s*Í¥ÄÎ¶¨Ïóê?/gi, '')
                         .replace(/Î£®Ìã¥\s*Í¥ÄÎ¶¨Ïùò?/gi, '')
                         .replace(/Î£®ÌåÖ\s*Í¥ÄÎ¶¨Ïùò?/gi, '')
                         .replace(/Î£®Ìã¥\s*Í¥ÄÎ¶¨Ìï¥??/gi, '')
                         .replace(/Î£®ÌåÖ\s*Í¥ÄÎ¶¨Ìï¥??/gi, '')
                         .replace(/Î£®Ìã¥[?òÏóê]?/gi, '')
                         .replace(/Î£®ÌåÖ[?òÏóê]?/gi, '')
                         .replace(/Í∏∞Î°ù??/gi, '')
                         .replace(/?ºÍ∏∞??/gi, '')
                         .replace(/?îÎç∞?¥Ïóê?/gi, '')
                         .replace(/Í∏∞ÎÖê?ºÏóê?/gi, '')
                         .replace(/Ï∂îÍ???s*Ï£ºÏÑ∏??gi, '')
                         .replace(/?Ä?•Ìï¥\s*Ï£ºÏÑ∏??gi, '')
                         .replace(/?£Ïñ¥\s*Ï£ºÏÑ∏??gi, '')
                         .replace(/Ï∂îÍ???/gi, '')
                         .replace(/?Ä?•Ìï¥?/gi, '')
                         .replace(/?Ä??gi, '')
                         .replace(/?£Ïñ¥Ï§?/gi, '')
                         ...
                         .replace(/Î∂Ä?ÅÌï¥/gi, '')
                         .replace(/??s*??s*??s*?¥Í≥†/gi, '')
                         .replace(/??s*??s*??s*?¥Ïñ¥Ï£ºÍ≥†/gi, '')
                         .trim();
                         
    if (textToParse) {
      const parsed = parseNaturalLanguageTodo(textToParse);
      
      const isRoutine = t.includes('Î£®Ìã¥') || t.includes('Î£®ÌåÖ');
      const isRecord = t.includes('Í∏∞Î°ù') || t.includes('?ºÍ∏∞');
      const isDday = t.includes('?îÎç∞??) || t.includes('Í∏∞ÎÖê??);
      ...
      let targetName = isRoutine ? 'Î£®Ìã¥' : (isRecord ? 'Í∏∞Î°ù' : (isDday ? '?îÎç∞?? : '????));
      if (statusText) {
        statusText.textContent = `??[${targetName}] Ï∂îÍ??? ${parsed.cleanedText}`;
      }
  ...
  async start(onStopCallback, onProgressCallback) {
    ...
    try {
      const stream = await navigator.mediaDevices.getUserMedia({ 
        audio: { sampleRate: 48000, channelCount: 2, echoCancellation: true, noiseSuppression: true } 
      });
...
function renderAudioPreviews(containerId, draftArray, onChangeCallback) {
  ...
  draftArray.forEach((audioData, idx) => {
    ...
    if (hasAudio) {
      ...
      dlAudioBtn.innerHTML = '?íæ';
      dlAudioBtn.title = '?§Îîî???Ä??;
      ...
      dlAudioBtn.addEventListener('click', () => {
        ...
        a.download = `?πÏùå_${new Date().getTime()}.webm`;
        a.click();
      });
      ...
      if (onChangeCallback) {
        ...
        delAudioBtn.innerHTML = '?óëÔ∏?;
        delAudioBtn.title = '?§Îîî????†ú';
    ...
    const hasRawText = audioData._rawTranscription ? true : false;

    if (!hasText && hasRawText) {
      const convertBtn = document.createElement('button');
      convertBtn.type = 'button';
      convertBtn.innerHTML = '??Í∏Ä??Î≥Ä?òÌïòÍ∏?;
    ...
    if (hasText) {
      ...
      textArea.placeholder = 'Î≥Ä?òÎêú ?çÏä§?∏Í? ?ÜÏäµ?àÎã§.';
      ...
      dlTextBtn.innerHTML = '?íæ';
      dlTextBtn.title = '?çÏä§???Ä??;
      ...
      dlTextBtn.addEventListener('click', () => {
        ...
        a.download = `?πÏùå?çÏä§??${new Date().getTime()}.txt`;
        a.click();
      });
      ...
      if (onChangeCallback) {
        ...
        delTextBtn.innerHTML = '?óëÔ∏?;
        delTextBtn.title = '?çÏä§????†ú';
        ...
        delTextBtn.addEventListener('click', () => {
          ...
          audioData._rawTranscription = null;
    ...
    if (AudioRecorder.isRecording) {
      ...
      AudioRecorder.start(
        (base64Audio, transcript) => {
          if (base64Audio) {
            draftsArray.push({ src: base64Audio, transcription: '', _rawTranscription: transcript });
            if (onChange) onChange();
          }
        },
        (interimTranscript) => {
          // Do nothing. Wait for explicit conversion button click.
        }
      );
    }
  });
...
const VideoRecorder = {
  ...
  async start(previewElement, onStopCallback) {
    if (this.isRecording) return;
    try {
      ...
      this.mediaRecorder.onstop = () => {
        const videoBlob = new Blob(this.videoChunks, { type: this.mediaRecorder.mimeType || 'video/webm' });
        const reader = new FileReader();
        reader.readAsDataURL(videoBlob);
        reader.onloadend = () => {
          const base64Video = reader.result;
          if (this.onStop) {
            this.onStop(base64Video);
          }
        };
        if (this.stream) {
      ...
      alert("Ïπ¥Î©î??Î∞?ÎßàÏù¥???ëÍ∑º Í∂åÌïú???ÑÏöî?©Îãà??");
    }
  },
...
function renderVideoPreviews(containerId, draftArray, onChangeCallback) {
  const container = document.getElementById(containerId);
  if (!container) return;
  container.innerHTML = '';
  draftArray.forEach((videoData, idx) => {
    const wrapper = document.createElement('div');
    wrapper.style.display = 'flex';
    wrapper.style.flexDirection = 'column';
    wrapper.style.gap = '4px';
    ...
    if (videoData.src) {
      ...
      if (onChangeCallback) {
        ...
        delVideoBtn.innerHTML = '?óëÔ∏?;
        delVideoBtn.title = '?ÅÏÉÅ ??†ú';
        delVideoBtn.style.background = 'none';
        delVideoBtn.style.border = 'none';
        delVideoBtn.style.cursor = 'pointer';
        delVideoBtn.style.fontSize = '1.2rem';
        delVideoBtn.addEventListener('click', () => {
          draftArray.splice(idx, 1);
          onChangeCallback();
        });
        actionCol.appendChild(delVideoBtn);
      }
      videoRow.appendChild(actionCol);
      wrapper.appendChild(videoRow);
      container.appendChild(wrapper);
    }
  });
}

function handleVideoUpload(inputId, draftsArray, onChange) {
  ...
  fileInput.addEventListener('change', (e) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const reader = new FileReader();
      reader.onload = (ev) => {
        draftsArray.push({ src: ev.target.result });
        if (onChange) onChange();
      };
      reader.readAsDataURL(file);
    }
...
function initVideoModals() {
  const videoModal = document.getElementById('video-record-modal');
  const btnClose = document.getElementById('btn-video-record-close');
  const btnStart = document.getElementById('btn-video-record-start');
  const btnStop = document.getElementById('btn-video-record-stop');
  const previewEl = document.getElementById('video-record-preview');
  
  let currentDraftsArray = null;
  ...
  if (btnStart) {
    btnStart.addEventListener('click', () => {
      ...
      VideoRecorder.start(previewEl, (base64Video) => {
        if (currentDraftsArray) {
          currentDraftsArray.push({ src: base64Video });
  ...
  const btnRecordNew = document.getElementById('btn-record-video');
  if (btnRecordNew) {
    btnRecordNew.addEventListener('click', () => {
      currentDraftsArray = state.diaryDraftVideo;
      currentOnChange = () => renderVideoPreviews('new-record-video-previews', state.diaryDraftVideo, () => renderVideoPreviews('new-record-video-previews', state.diaryDraftVideo, null));
      btnStart.style.display = 'flex';
      btnStop.style.display = 'none';
      ...
      navigator.mediaDevices.getUserMedia({ video: { width: { ideal: 640 }, height: { ideal: 480 } }, audio: true })
        .then(stream => {
          ...
          alert("Ïπ¥Î©î??Î∞?ÎßàÏù¥??Í∂åÌïú???ÑÏöî?©Îãà??");
        });
  ...
  const btnRecordTodo = document.getElementById('btn-record-todo-video');
  if (btnRecordTodo) {
    btnRecordTodo.addEventListener('click', () => {
      currentDraftsArray = todoEditDraftVideo;
      currentOnChange = () => renderVideoPreviews('todo-edit-modal-video-previews', todoEditDraftVideo, () => renderVideoPreviews('todo-edit-modal-video-previews', todoEditDraftVideo, null));
      btnStart.style.display = 'flex';
      btnStop.style.display = 'none';
  ...
  handleVideoUpload('new-record-video-input', state.diaryDraftVideo, () => renderVideoPreviews('new-record-video-previews', state.diaryDraftVideo, () => renderVideoPreviews('new-record-video-previews', state.diaryDraftVideo, null)));
  handleVideoUpload('todo-edit-modal-video-input', todoEditDraftVideo, () => renderVideoPreviews('todo-edit-modal-video-previews', todoEditDraftVideo, () => renderVideoPreviews('todo-edit-modal-video-previews', todoEditDraftVideo, null)));
}

...
function renderTodos() {
  ...
  sortedTodos.forEach(todo => {
    ...
    textSpan.title = '클릭: 일정 공유/추가 | 길게 누르기: 수정';
    textSpan.style.cursor = 'text';

    // Long press support (Mouse + Touch)
    let pressTimer = null;
    let startX = 0;
    let startY = 0;

    const startPress = (e) => {
      if (e.type === 'mousedown' && e.button !== 0) return; // Only left clicks for mouse simulation
      longPressed = false;
      ...
      pressTimer = setTimeout(() => {
        ...
        longPressed = true;
        openTodoEditModal(todo.id);
      }, 400); // 400ms hold (more responsive!)
    };
    ...
    const movePress = (e) => {
      ...
      if (diffX > 10 || diffY > 10) { // 10px movement tolerance
        cancelPress();
      }
    };
    ...
    textSpan.addEventListener('touchstart', startPress, { passive: true });
    textSpan.addEventListener('touchend', cancelPress);
    textSpan.addEventListener('touchmove', movePress, { passive: true });
    textSpan.addEventListener('touchcancel', cancelPress);

    textSpan.addEventListener('mousedown', startPress);
    textSpan.addEventListener('mouseup', cancelPress);
    textSpan.addEventListener('mouseleave', cancelPress);
    textSpan.addEventListener('mousemove', movePress);
